...
export function recommendSongs() {
  return api.get(`/recommend/songs`).then((res: any) => {
...
export function queryArtistSublist() {
  return api.get(`/artist/sublist`).then((res: any) => {
    return res;
  });
}
...
export function recommendResource() {
  return api.get(`/recommend/resource`).then((res: any) => {
...
export function getAlbumNewest() {
  // 标准版返回
  return api.get(`/album/newest`).then((res: any) => {
    res.albums.forEach((it: albumTypes) => {
      it.resourceType = 'ALBUM';
    });
    return res;
...
export function queryAlbumSublist() {
  // 标准版返回
  return api.get(`/album/sublist`).then((res: any) => {
...
export function getSongDounloadUrl(id: number) {
  return api.get(`/song/download/url`, {
    params: {
      id,
      br: 320000,
    },
